...
describe('memory_switch_project', () => {
  ...
  beforeEach(() => {
    ...
    registerMemorySwitchProject(server, session as Session);
  });

  it('should switch project', async () => {
    const result = await toolHandler({ name: 'other-project' });

    expect(result.content[0]?.text).toContain('other-project');
    expect(session.switchProject).toHaveBeenCalledWith('other-project');
    expect(session.saveConfig).toHaveBeenCalled();
  });

  it('should handle errors', async () => {
    session.switchProject = jest.fn().mockImplementation(() => {
